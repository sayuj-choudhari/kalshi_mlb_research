...
class SpreadVisualizer:
    ...
    def get_game_start_time(self, event_ticker: str, games_csv: str = 'games.csv') -> Optional[datetime]:
        ...
        for path in possible_paths:
            if os.path.exists(path):
                games_csv_path = path
                if self.verbose:
                    print(f"[FOUND] Found games.csv at: {games_csv_path}")
                break
        
        if games_csv_path is None:
            if self.verbose:
                print(f"[WARNING] Could not find games.csv in: {possible_paths}")
            return None
        ...
        try:
            ...
            if len(parts) < 2:
                if self.verbose:
                    print(f"[WARNING] Could not parse event ticker: {event_ticker}")
                return None
            ...
            if len(date_part) >= 6:
                try:
                    ...
                    if self.verbose:
                        print(f"[SEARCHING] Looking for games on: {game_date.date()}")
                    ...
                    if self.verbose:
                        print(f"[FOUND] Found {len(matching_games)} games on that date")
                    ...
                    if not matching_games.empty:
                        ...
                        if self.verbose:
                            print(f"[INFO] Game start time: {game_start}")
                        return game_start
                    else:
                        if self.verbose:
                            print(f"[WARNING] No games found for date {game_date.date()}")
                except Exception as e:
                    if self.verbose:
                        print(f"[WARNING] Error parsing date from ticker: {e}")
                    pass
            
            return None
        except Exception as e:
            if self.verbose:
                print(f"[WARNING] Error reading games.csv: {e}")
            return None
    ...
    def plot_spread_and_volume(self, csv_file: str, pair_filter: Optional[str] = None, save_path: Optional[str] = None, show_all_pairs: bool = False):
        ...
        if self.verbose and game_start_time is not None:
            print(f"[GAME START] Game start time (PT, naive): {game_start_time}")
        elif self.verbose:
            print(f"[WARNING] Could not find game start time for {game}")
        ...
        for idx, arb_type in enumerate(sorted(arb_types)):
            ...
            if self.verbose and len(subset) > 0:
                print("[DEBUG] First 3 spread event start times (ET, naive):")
    ...
    def plot_all_spreads_timeline(self, csv_file: str, pair_filter: Optional[str] = None, 
        ...
        if game_start_time_raw is not None:
            ...
            if ts.tzinfo is not None:
                ...
                game_start_time = ts.tz_localize('UTC').tz_convert('US/Pacific').tz_localize(None)
        if self.verbose and game_start_time is not None:
            print(f"[GAME START] Timeline plot - Game start time (PT, naive): {game_start_time}")
        ...
        if save_path:
            plt.savefig(save_path, dpi=300, bbox_inches='tight')
            print(f"\n[OK] Spread vs time plot saved to: {save_path}")
        else:
            plt.show()
...
Examples:
  ...
  python spread_visualizer.py --event KXMLBGAME-25SEP28BALNYY --threshold 0.02 --save plot.png
    ...
    if args.event and args.threshold is not None:
        ...
        if not csv_file:
            print(f"[ERROR] CSV file not found for {args.event} with threshold {args.threshold}")
            return
        
        if args.timeline:
            # Timeline plot
            visualizer.plot_all_spreads_timeline(csv_file, pair_filter=args.pair, save_path=args.save, 
                                               show_all_pairs=args.all_pairs)
        else:
            # Spread and volume plot
            visualizer.plot_spread_and_volume(csv_file, pair_filter=args.pair, save_path=args.save, 
                                             show_all_pairs=args.all_pairs)
    else:
